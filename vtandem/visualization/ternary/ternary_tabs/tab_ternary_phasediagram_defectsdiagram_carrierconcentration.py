...
from vtandem.visualization.ternary.ternary_windows.window_ternary_defectsdiagram import Window_Ternary_DefectsDiagram
...
class Tab_Ternary_PhaseDiagram_DefectsDiagram_CarrierConcentration(QWidget):
	...
	def __init__(self, main_compound=None, first_element=None, second_element=None, third_element=None, compounds_info=None, defects_data=None, dos_data=None, show_defects_diagram = True, show_carrier_concentration = True):
		...
		if self.show_defects_diagram:
			...
			self.defectsdiagram_window = Window_Ternary_DefectsDiagram(	first_element = self.first_element, \
																		second_element = self.second_element, \
																		third_element = self.third_element, \
																		main_compound = self.main_compound, \
																		DefectsDiagram_Plot = self.DefectsDiagram, \
																		show_carrier_concentration = self.show_carrier_concentration	)
			...
			self.tab1_defectsdiagram_widget_layout.addWidget(self.dopant_properties_widget)
	...
	def Generate_PhaseDiagram_Plot_Function(self, event):
		...
		# Reset clicked point
		self.pressed_point_plot, = self.PhaseDiagram.ternary_phase_diagram_plot_drawing.plot([0.0], [0.0], color=self.pressed_point_desc['color'], marker=self.pressed_point_desc['marker'])
		...
		if self.show_defects_diagram:
			...
			self.defectsdiagram_window.DefectsDiagram_Plot.first_element	= self.first_element
			self.defectsdiagram_window.DefectsDiagram_Plot.second_element	= self.second_element
			self.defectsdiagram_window.DefectsDiagram_Plot.third_element	= self.third_element
			self.defectsdiagram_window.DefectsDiagram_Plot.mu_elements[self.first_element]["mu0"] = self.compounds_info[self.first_element]["mu0"]
			self.defectsdiagram_window.DefectsDiagram_Plot.mu_elements[self.second_element]["mu0"] = self.compounds_info[self.second_element]["mu0"]
			self.defectsdiagram_window.DefectsDiagram_Plot.mu_elements[self.third_element]["mu0"] = self.compounds_info[self.third_element]["mu0"]
			self.defectsdiagram_window.DefectsDiagram_Plot.mu_elements[self.first_element]["deltamu"] = self.deltamu_values[self.first_element]
			self.defectsdiagram_window.DefectsDiagram_Plot.mu_elements[self.second_element]["deltamu"] = self.deltamu_values[self.second_element]
			self.defectsdiagram_window.DefectsDiagram_Plot.mu_elements[self.third_element]["deltamu"] = self.deltamu_values[self.third_element]
			
			
			self.defectsdiagram_window.Generate_DefectsDiagram_Plot_Function()
	...
	def Update_ExtrinsicDefect(self):
		...
		if self.extrinsic_defect_selection_box.currentText() == "None":
			self.extrinsic_defect = "None"
			self.extrinsic_defect_chemical_potential_deltamu.setEnabled(False)
		else:
			dopant_qcombobox_text = self.extrinsic_defect_selection_box.currentText()
			self.extrinsic_defect = dopant_qcombobox_text.split("->")[0]+"_"+dopant_qcombobox_text.split("->")[1]
			self.extrinsic_defect_chemical_potential_deltamu.setEnabled(True)
