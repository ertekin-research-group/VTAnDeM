...
class Quaternary_Carrier_Concentration(object):
	...
	def __init__(self, parent = None, main_compound = None, first_element = None, second_element = None, third_element = None, fourth_element = None):
		...
		"""
		self.carrier_concentration_hole_plot = None
		self.carrier_concentration_electron_plot = None
		"""
	...
	def Calculate_Defect_Carrier_Concentration(self):
		...
		for defect in self.quaternary_defects_data.keys():
			...
			if self.quaternary_defects_data[defect]["Extrinsic"] == "No":
				...
				for charge in self.quaternary_defects_data[defect]["charge"].keys():
					...
					elif self.fourth_element == defect.split("_")[-1]:
		...
		for charge in self.quaternary_defects_data[self.extrinsic_defect]["charge"].keys():
			
			# Formation energy
			"""
			extrinsic_defect_formation_enthalpy = self.quaternary_defects_data[self.extrinsic_defect]["charge"][charge]["Energy"] \
													- self.quaternary_defects_data["supercellsize"] * self.main_compound_total_energy \
													- self.quaternary_defects_data[self.extrinsic_defect]["n_"+self.species_a] * ( self.first_element_mu0 + float(dmu_a) ) \
													- self.quaternary_defects_data[self.extrinsic_defect]["n_"+self.species_b] * ( self.second_element_mu0 + float(dmu_b) ) \
													- self.quaternary_defects_data[self.extrinsic_defect]["n_"+self.species_c] * ( self.third_element_mu0 + float(dmu_c) ) \
													- self.quaternary_defects_data[self.extrinsic_defect]["n_"+self.species_d] * ( self.fourth_element_mu0 + float(dmu_d) ) \
													- (self.extrinsic_defect_mu0 + self.extrinsic_defect_deltamu) \
													+ float(charge) * self.fermi_energy_array \
													+ self.quaternary_defects_data[self.extrinsic_defect]["charge"][charge]["ECorr"]
			"""
			extrinsic_defect_formation_enthalpy = self.quaternary_defects_data[self.extrinsic_defect]["charge"][charge]["Energy"] \
													- self.quaternary_defects_data["supercellsize"] * self.main_compound_total_energy \
													- (self.extrinsic_defect_mu0 + self.extrinsic_defect_deltamu) \
													...
													+ self.quaternary_defects_data[self.extrinsic_defect]["charge"][charge]["ECorr"]
			for element in self.elements_list:
				extrinsic_defect_formation_enthalpy -= self.quaternary_defects_data[self.extrinsic_defect]["n_"+element] * ( self.mu_elements[element]["mu0"] + self.mu_elements[element]["deltamu"] )
	...
	def Calculate_CarrierConcentration(self):
		...
		"""
		self.total_hole_concentration = []
		self.total_electron_concentration = []
		"""
	...
	def Initialize_HoleConcentration_Plot(self):
		...
		intrinsic_defect_hole_concentration, intrinsic_defect_electron_concentration, total_hole_concentration, total_electron_concentration = self.Calculate_CarrierConcentration()
			...
			while equilibrium_fermi_energy == 0.0:
				...
				for defect in self.quaternary_defects_data.keys():
					...
					if self.quaternary_defects_data[defect]["Extrinsic"] == "No":
						for charge in self.quaternary_defects_data[defect]["charge"].keys():
							"""
							intrinsic_defect_formation_enthalpy = self.quaternary_defects_data[defect]["charge"][charge]["Energy"] \
																- self.quaternary_defects_data["supercellsize"] * self.main_compound_total_energy \
																- self.quaternary_defects_data[defect]["n_"+self.species_a] * ( self.first_element_mu0 + float(dmu_a) ) \
																- self.quaternary_defects_data[defect]["n_"+self.species_b] * ( self.second_element_mu0 + float(dmu_b) ) \
																- self.quaternary_defects_data[defect]["n_"+self.species_c] * ( self.third_element_mu0 + float(dmu_c) ) \
							...
							intrinsic_defect_formation_enthalpy = self.quaternary_defects_data[defect]["charge"][charge]["Energy"] \
																...
																+ self.quaternary_defects_data[defect]["charge"][charge]["ECorr"]
							for element in self.elements_list:
								intrinsic_defect_formation_enthalpy -= self.quaternary_defects_data[defect]["n_"+element] * ( self.mu_elements[element]["mu0"] + self.mu_elements[element]["deltamu"] )
							if self.first_element == defect.split("_")[-1]:
								N = self.main_compound_number_first_specie/self.vol
							elif self.second_element == defect.split("_")[-1]:
								N = self.main_compound_number_second_specie/self.vol
