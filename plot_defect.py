...
__name__ = 'VTAnDeM_Visualization-Toolkit-for-the-Analysis-of-Defects-in-Materials'
...
# 1) Fixed generate phase diagram and defects diagram connection
# 2) Fixed species button connection to fourth species slider
# 3) Fixed mu value displays/calculations
...
class Quaternary_ApplicationWindow(QMainWindow):
	...
	def __init__(self, parent = None, main_compound = None, first_species = None, second_species = None, third_species = None, fourth_species = None):	# User specifies the main compound and its constituents
		...
		"""
		# Establish fullscreen geometries to reference the sizes of all other widgets
		...
		# Establish a variable for the main quaternary compound
		...
		# Endpoints for quaternary phase diagram
		...
		self.mu4 = -0.0											# Current value of mu4 (will be updated as user utilizes the slider bar)
		...
		self.widgets_grid = QGridLayout(self.widgets)	# In the comment for self.widgets, we say that the programmer needs to place all of our
														...
														# Example: A widget with (x,y,h,w)=(0,0,1,2) in a 3x3 grid would look like:
														#	 _ _ _             _ _ _
														#	|     |           |O O  |
														#	|     |   ===>    |     |
														#	|_ _ _|           |_ _ _|
		...
		self.widgets_grid.addWidget(self.quaternary_compound_title, 0, 0, 1, 4)				# Add the widget to the "main" widget grid layout
	...
	def Activate_MuValue_Displays(self):
		...
		self.display_label_font = QFont("Arial", 12)						# Define the font
	...
	def Activate_PhaseDiagram_Plot_Axes(self):
		...
		self.quaternary_plot_drawing.set_xlabel("$\Delta\mu_{a}$ (eV)",fontdict=self.font,labelpad=8)
		self.quaternary_plot_drawing.set_ylabel("$\Delta\mu_{b}$ (eV)",fontdict=self.font,rotation=270,labelpad=16)
	...
	def Update_PhaseDiagram_Plot_Axes(self):
		...
		self.quaternary_plot_drawing.set_xlabel("$\Delta\mu_{"+self.first_species+"}$ (eV)",fontdict=self.font,labelpad=8)
		self.quaternary_plot_drawing.set_ylabel("$\Delta\mu_{"+self.second_species+"}$ (eV)",fontdict=self.font,rotation=270,labelpad=16)
	...
	def Activate_Generate_PhaseDiagram_Plot_Button(self):
		...
		self.generate_quaternary_plot_button_label = QLabel("Hello. Welcome to VTAnDeM.")	# A heart-warming welcome message that is only visible at the start of the app
		...
		self.generate_quaternary_plot_button = QPushButton("Generate Plot!")							# Create the button widget that the user will press to generate a new phase diagram
	...
	def Generate_PhaseDiagram_Plot(self, event):
		...
		if len(self.species) > len(set(self.species)):	# Every time someone clicks the species' buttons, the self.species list gets updated.
			...
			self.generate_quaternary_plot_button_label.setText("Yo... Pick UNIQUE elements!")	# Maybe change the phrasing ;)
	...
	def Activate_DefectsDiagram_Plot_Axes(self):
		...
		self.defects_diagram_plot_drawing.set_ylim(-0.05, 1.0)
		...
		self.defects_diagram_plot_drawing.xaxis.labelpad = 2.0
		self.defects_diagram_plot_drawing.yaxis.labelpad = 1.5		
	...
	def Activate_CarrierConcentration_Plot_Axes(self):
		...
		self.carrier_concentration_plot_drawing.set_ylim((0, 1.0))		
		self.carrier_concentration_plot_drawing.set_xlabel("T(K)", fontdict=self.font)
		self.carrier_concentration_plot_drawing.set_ylabel("ni" +"(cm$^{-3}$)", fontdict=self.font, rotation=90)
		...
		plt.subplots_adjust(left = 0.25, right = 0.9, top = 0.95, bottom = 0.15)
		self.carrier_concentration_plot_drawing.xaxis.labelpad = 2.0
		self.carrier_concentration_plot_drawing.yaxis.labelpad = 1.0
